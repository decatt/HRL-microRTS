...
from feudal_agent import ScriptManager
import time
import torch.nn as nn
import matplotlib.pyplot as plt
import collections
from lower_level_script import near_target, get_default_action, move_to_target_and_act
...
worker = ScriptManager().to(device)
if load:
...
invalid_action_masks = torch.zeros((num_steps, num_envs) + (512,)).to(device)
goals = torch.zeros((num_steps, num_envs) + (2,)).to(device)
...
for update in range(starting_update, num_updates + 1):
    ...
    for step in range(0, num_steps):
        ...
        with torch.no_grad():
            ...
            unit_mask = torch.cat((torch.Tensor(np.array(envs.vec_client.getUnitLocationMasks()).reshape(num_envs, -1)), torch.ones((num_envs, 256))),dim=1)
            goal, log_prob, _, invalid_action_masks[step] = worker.get_goal(next_obs, unit_mask)
            goals[step] = goal.T
            log_probs[step] = log_prob
            action_mask = np.array(envs.vec_client.getUnitActionMasks(goal[0].cpu().numpy())).reshape(num_envs, -1)
            action = torch.zeros((num_envs, 8)).to(device)
            for i in range(num_envs):
                selected_unit = goal[0][i].tolist()
                target = goal[1][i].tolist()
                action_temp = action_mask[i]
                selected_unit_x = selected_unit%16
                selected_unit_y = selected_unit//16
                a = torch.Tensor(move_to_target_and_act(next_obs[i], selected_unit, target, action_temp, 16)).to(device)
                action[i] = a
            next_obs, rs, ds, infos = envs.step(action.int())
    ...
    with torch.no_grad():
        last_value = worker.get_value(next_obs.to(device)).reshape(1, -1)
        if gae:
    ...
    for i_epoch_pi in range(update_epochs):
        ...
        for start in range(0, batch_size, minibatch_size):
            ...
            _, new_log_prob, entropy, _ = worker.get_goal(
                b_obs[minibatch_ind],
                action=b_actions.long()[minibatch_ind].T,
                source_unit_mask=b_invalid_action_masks[minibatch_ind])
    ...
    if update % 64 == 0:
        date = time.strftime("%Y%m%d%H", time.localtime())
        path = './model/microrts_script_fun_' + date + '_worker.pth'
        path_pt = './model/microrts_script_fun_' + date + '_worker.pt'
        record_path = './record/microrts_script_fun_' + date + '.txt'
